...
export function RecordingControls(props: any) {
  ...
  React.useLayoutEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      // Note: Can not use arrow key as they are used by Babylon JS and the 3D Camera
      // TODO: on "pause" keypress is sometimes missed and sometimes jumping around brakes everything
      if (e.repeat) return;
      if (e.key === "5") {
        if (play) {
          console.log("msg to paus");
          ipcClient.sendMessage("pause_rec", null, (res: any) => {
            console.log("Pausing Cb!");
